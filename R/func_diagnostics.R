...
compute.sres<-function(saemixObject) {
  ...
  if(length(saemixObject["sim.data"]["N"])==0 || saemixObject["sim.data"]["nsim"]!=nsim) {
    if(saemixObject["options"]$warnings) cat("Simulating data using nsim =",nsim,"simulated datasets\n")
  ...
  #  pde<-c()
  if(saemixObject["options"]$warnings) cat("Computing WRES and npde ")
  for(isuj in 1:saemixObject["data"]["N"]) {
    if(isuj%%10==1) {if(saemixObject["options"]$warnings) cat(".")}
...
#' @return a list containing 4 components: 
...
testnpde<-function(npde) 
{
  ...
  names(myres)<-c("  t-test               ", "  Fisher variance test ", "  SW test of normality ","  Global test          ")
